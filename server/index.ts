...
async function main(): Promise<void> {
        const server = new Server({ name: 'mcp-ardf-demo', version: '0.1.0' });
        ...
        server.registerTool(
                'prompt.run',
                z.object({ name: z.string(), vars: z.record(z.string()).optional() }),
                async ({ name, vars }) => {
